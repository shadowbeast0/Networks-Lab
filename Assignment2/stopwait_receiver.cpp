...
int main(int argc, char** argv) {
    winsock_init();
    double p_err = (argc >= 2 ? std::stod(argv[1]) : 0.0);  // apply errors to ACKs
    ...
    while (true) {
        // We don't know the exact frame size, so read the minimum then extend by polling.
        // For simplicity, assume frames are <= 2048 total.
        int timeout_ms = 60000;
        if (!recv_exact(s, buf.data(), 15 + MIN_PAYLOAD + 4, timeout_ms)) {
            std::cout << "[RECV] Connection closing or no more data.\n"; break;
        }
        // Try to read any extra bytes quickly (non-blocking)
        ...
        if (!parsed) { std::cout << "[RECV] Bad frame parse -> drop\n"; continue; }
        std::cout << "[RECV] Frame seq=" << int(f.seq) << " len=" << f.length
                  << " (got " << have << " bytes), CRC=" << (ok_crc?"OK":"BAD") << "\n";

        if (ok_crc && f.seq == expected) {
            expected = uint8_t(expected + 1);
            // send ACK for this sequence
            Ack a{ACK, f.seq};
