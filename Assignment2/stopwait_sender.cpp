...
static const size_t   MAX_FRAME = 15 + MIN_PAYLOAD + 4 + 1500; // generous buffer

// Reads next payload line (ASCII) and returns bytes (padded to >=46)
...
int main(int argc, char** argv) {
    winsock_init();
    double p_err = (argc >= 2 ? std::stod(argv[1]) : 0.0); // bit error prob (0..0.5)
    int max_delay = (argc >= 3 ? std::stoi(argv[2]) : 0);  // ms
        ...
        while (!acked) {
            ...
            if (recv_exact(conn, ackbuf, sizeof(ackbuf), int(rtt.rto_ms))) {
                ...
                if (Ack::parse(ackbuf, sizeof(ackbuf), a) && a.type==ACK && a.seq==seq) {
                    ...
                    rtt.observe(ms);
                    std::cout << "[SENDER] ACK " << int(a.seq) << " (RTT=" << ms
                              << "ms, RTO=" << rtt.rto_ms << "ms)\n";
                    acked = true;
                    seq = uint8_t(seq + 1);
                    break;
                } else {
