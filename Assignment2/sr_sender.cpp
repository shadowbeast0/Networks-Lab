...
int main(int argc, char** argv) {
    ...
    double p_err = (argc >= 3 ? std::stod(argv[2]) : 0.0);    // bit error
    ...
    uint8_t base = 0; // lowest outstanding
    uint8_t nextseq = 0;
    size_t idx = 0;

    struct Slot { bool in_use=false, acked=false; std::vector<uint8_t> wire; std::chrono::steady_clock::time_point deadline; };
    std::map<uint8_t, Slot> window;

    auto in_window = [&](uint8_t s){ int diff = int(uint8_t(s - base)); return 0 <= diff && diff < N; };
    ...
    auto push_new = [&](){
        while (in_window(nextseq) && idx < payloads.size()) {
            ...
            f.seq = nextseq; f.payload = payloads[idx];
            Slot slot; slot.in_use=true; slot.acked=false; slot.wire = f.serialize_with_crc();
            window[nextseq] = std::move(slot);
            send_or_resend(nextseq,false);
            ++idx; nextseq = uint8_t(nextseq + 1);
        }
    };

    push_new();

    // main loop: wait for ACK/NAK or timeouts
    while (!window.empty()) {
        // 1) non-blocking check for ACK/NAK
        u_long nb=1; ioctlsocket(conn, FIONBIO, &nb);
        uint8_t ackbuf[6]; int r = recv(conn, reinterpret_cast<char*>(ackbuf), sizeof(ackbuf), 0);
        nb=0; ioctlsocket(conn, FIONBIO, &nb);
        ...
        // Replace std::this_thread::sleep_for with WinAPI Sleep to avoid std::this_thread usage
        Sleep(10); // milliseconds
    }

    std::cout << "[SR SENDER] All frames delivered.\n";
    closesocket(conn); closesocket(ls); winsock_cleanup(); return 0;
}
